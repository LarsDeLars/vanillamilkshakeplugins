...
public class TpAskHere implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(sender instanceof Player){
            ...
            TeleportRequest request = new TeleportRequest(p, target, true, 120);

            TeleportRequest.teleportRequests.removeIf(r -> r.getAsked() == p);
            TeleportRequest.teleportRequests.add(request);

            p.sendMessage(ChatColor.GREEN + "Asked " + target.getDisplayName() + ChatColor.GREEN + " to teleport to you. To cancel this request use " + ChatColor.RED + "/tpcancel");
            target.sendMessage(p.getDisplayName() + ChatColor.GOLD + " aks you to teleport to " + ChatColor.BOLD + "them" + ChatColor.GOLD + ", use " + ChatColor.GREEN + "/tpaccept " + ChatColor.GOLD + "or " + ChatColor.RED + "/tpdeny");
